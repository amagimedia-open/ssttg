...
#seehttps://github.com/googleapis/python-speech/blob/master/UPGRADING.md#enums-and-types
...
class StreamDataStructure ():
    ...
    def get_mapped_audio_pts (self, inp_pts):
        ...
        #eprint (f"$$$ ----{self.audio_pts_map[key]}  ---- {self.audio_pts_map[key] + inp_pts - key}")
    ...
    def get_data_from_pts (self):
        ...
        eprint (f"get_data_from_pts ,pts={pts} hpts={hpts} tpts={tpts} == {pts < hpts or pts > tpts and tpts-pts >= G_CHUNK_MS}")
        ...
        while self.sent_audio_q.qsize () > 0:
            ...
            data_l.append (data)
            self.sent_q_head_pts += G_CHUNK_MS 
            #eprint (f"Getting a CHUNKKKKKKKKKKK {len(data)}")

        if data_l:
            data = b''.join(data_l)
            
        self.old_data_sent_ms = len(data) / G_BYTE_PER_SAMPLE / (G_AUD_SAMPLING_RATE/1000)
        eprint (f"Resending {self.old_data_sent_ms} ms data")

        return data
        

class myThread (threading.Thread):
    def __init__(self, threadID, name, q,srt_fname, final_flag_srt):
        ...
        self.final_flag_srt = final_flag_srt
  
    def srt_writer (self, text, tc_in, duration_ms, is_final=False):
        #--with open (self.srt_fname, "w") as fp:
        with open (self.srt_fname, "a") as fp:
            #fp = sys.stdout
            if is_final:
                srt_text = f"#F\n"
            else:
            ...
            #fp.write (srt_text.encode ('ascii', 'ignore').decode('ascii').upper())
            fp.write (srt_text.encode ('ascii', 'ignore').decode('ascii'))
            eprint (srt_text.encode('utf-8'))
    ...
    def align_to_next_word (self, cur_transcription, is_timeout=False):
        ...
        if len (cur_transcription) > last_len or is_timeout:
            if cur_transcription[:last_len] == self.last_transcription:
                ...
                if i_in_new != -1:
                    ...
                    eprint(f"==Fixing misalignment :: old:{self.last_read_pos} new:{new_read_pos} {self.last_transcription[i:last_len].encode('utf-8')} {i_in_new}")
                else:
                ...
                if new_read_pos != -1:
                    ...
                    return -1
                eprint (f"Got misalignment, aligning to {new_read_pos}, test={cur_transcription[new_read_pos:].encode('utf-8')}")
                #eprint (f"last={self.last_transcription.encode('utf-8')}")
                #eprint (f"curr={cur_transcription.encode('utf-8')}")
        ...
        if len(cur_transcription)- self.last_read_pos > G_MAX_SUB_CHARS:
            ...
            if i <= self.last_read_pos:
                eprint ("Something went wrong, no space character found. Cutting abruptly.")
    ...
    def get_duration (self, end_pos, durms=0):
        ...
        #eprint (self.pos_pts_map)
        #eprint (f"==ks={self.last_read_pos}==ke={end_pos}===e={key_end} - s={key_start}========{self.pos_pts_map[key_end] - self.pos_pts_map[key_start]}")
    ...
    def write (self, cur_transcription, durms=0, is_final=0):
        if len (cur_transcription) > self.last_read_pos:
            ...
            #eprint (f"tc_in={tc_in}  durms= {durms}")
            ...
            #eprint (f"==== {end_pos} ==== {self.last_transcription[:end_pos]}")
            return 0
        return 0
    ...
    def run(self):
        ...
        while not self.exit_flag:
            try:
                ...
                #eprint (f"exception {get_current_time ()}")
                if len(self.pos_pts_map) == 0 or not self.last_transcription:
                    continue
                #cur_transcription = result.alternatives[0].transcript
                #cur_timestamp = result.result_end_time.seconds*1000 + result.result_end_time.microseconds/1000
            ...
            try:
                ...
                #eprint (f"----{x} -------- {cur_timestamp}")
                
                if result.is_final:
                    eprint (f"=FINAL===={cur_transcription.encode('utf-8')}=====")
                    while len(cur_transcription) - self.last_read_pos > 1:
                        ret = self.write (cur_transcription.strip(), 0, True)
                        if ret == -1:
                            break 
                    if self.final_flag_srt and \
                       len (result.alternatives[0].transcript.strip()) > 0:
                        tc_in = self.pos_pts_map[next(iter(self.pos_pts_map))]
                        durms = x - tc_in
                        self.srt_writer (result.alternatives[0].transcript.strip(), \
                                tc_in, durms, True)
...
class ReadGen(threading.Thread):
    ...
    def re_align (self,old_data, sync_pos):
        if len(old_data) - sync_pos >= G_AUDIO_HEADER_LEN:
            ...
            eprint (f"{data_len} ol={old_data_len} ... ")
    ...
    def run (self):
        while not self.exit_flag:
            ...
            if now - self.last_log_time >= 5:
                ...
                eprint (f"Data read in last 5000ms is {self.data_read}ms")
                self.data_read = 0
                self.last_log_time = now
            if data and data[0:4].hex() != 'c0ffeeee':
                eprint ("Could not get Sync bytes.")
                pos = self.get_sync_byte_position (data)
                if pos == -1:
                    eprint ("Could not get Sync bytes in the entire packet, dropping it.")
                    continue
                else:
                    eprint (f"Re-aligning ...{pos}.......{data[pos:pos+5].hex()}....{audio_header_get_pts (data[pos:])}.........")
...
class GeneratorClass ():
    ...
    def parse_audio_packet (self, data):
        if data and data[0:4].hex() != 'c0ffeeee':
            eprint (f"Could not get Sync bytes. Aborting...{data[0:4].hex()}")
            os._exit (0)
        #eprint (f"{data[4:9].hex()} ========")
        ...
        #eprint (f"pts:{pts} len:{data_len} {len(data[G_AUDIO_HEADER_LEN:data_len+G_AUDIO_HEADER_LEN])}========")
        #eprint(data[10:data[9]])
    ...
    def get_bytes (self):
        ...
        eprint (f"Exiting generator, bytes put = {self.stream.consumed_ms}")

def main(audio_pipe_fname, srt_pipe_srt_fname, phrases_fname, final_flag_srt):
    ...
    thread_obj = myThread (1, "audio_to_srt_1", q_obj, srt_pipe_srt_fname, final_flag_srt)
    thread_obj.start ()
    ...
    try:
        ...
        eprint ("Export authorization json in environment")
    ...
    while True:
        try:
            ...
            eprint (f"===============RETRY AFTER 5MIN======last_sent={stream_obj.last_sub_pts}")
            if len(stream_obj.audio_pts_map.keys ()) > 0:
                lk = list(stream_obj.audio_pts_map.keys ())[-1]
                eprint (f"===audio_pts_map[{lk}] = {stream_obj.audio_pts_map[lk]}===========")

        except google.api_core.exceptions.ServiceUnavailable:
            eprint ("===============ServiceUnavailable exception.===RETRY===================")
...
if __name__ == '__main__':
    ...
    for o, v in opts:
        if o in ("-h", "--help"):
            ...
            opt_cfg_path = v
        elif o in ("-f", "--finalsrt"):
            opt_final_flag_srt = True
        elif o in ("-v", "--verbose"):
            opt_verbose = True
        elif o in ("-n", "--norun"):
            opt_no_run = True
    ...
    if (not G_FILES_SECTION_NAME in config):
        config.add_section(G_FILES_SECTION_NAME)
    config.set(G_FILES_SECTION_NAME, G_PHRASES_PATH_CFG_NAME, "")
    if (len(opt_input_path)):
        config.set(G_FILES_SECTION_NAME, G_INPUT_AUDIO_PATH_CFG_NAME, opt_input_path)
    if (len(opt_output_path)):
        config.set(G_FILES_SECTION_NAME, G_OUTPUT_SRT_PATH_CFG_NAME, opt_output_path)
    if (len(opt_auth_path)):
        config.set(G_FILES_SECTION_NAME, G_GCP_AUTH_PATH_CFG_NAME, opt_auth_path)
